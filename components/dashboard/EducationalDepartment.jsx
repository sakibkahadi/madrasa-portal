...
const educationalDepartment = [
  { 
   ...
   { name: "resultSheet", banglaName: "শিক্ষার্থীদের ", icon: FileText, gradient: "bg-gradient-to-r from-yellow-500 to-amber-500", slug: "result-sheet" },
...
export default function EducationalDepartment() {
  const { t } = useLanguage();
  const router = useRouter();

  const handleCardClick = (slug) => {
    router.push(`educational-department/${slug}`);
  };
      ...
      <Tabs>
        <TabsContent>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mt-8 sm:mt-0">
            {educationalDepartment.map((card, index) => (
              <Card
                key={index}
